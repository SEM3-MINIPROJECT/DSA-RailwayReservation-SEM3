...
void displayAvailableSeats(struct Reservation* head) {
    ...
    for (int row = 1; row <= 7; row++) {
        // Print side decoration and 6 seats on one side
        printf("| ");
        for (int seat = 1; seat <= 6; seat++) {
            if (current != NULL) {
                 if(current->isReserved==false){
                  printf("%3d ", current->seatNumber);
                 }
                 else printf("  X ");
        ...
        // Print space between seat sides
        printf("      ");

        // Print 2 seats on the other side and side decoration
